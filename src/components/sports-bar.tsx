...
import { buildBasketballStandings, buildVolleyballStandings } from "@/lib/standings-calc";

type SportKey = "Football" | "Basketball" | "Volleyball" | "Swimming";
type SectionKey = "Home" | "Matches" | "Standings" | "Teams" | "Players";
type BFilter = "U15 Boys" | "U15 Girls";
...
export default function SportsBar() {
  ...
  const basketballStandings = useMemo(() => buildBasketballStandings(), []);
  const volleyballStandings = useMemo(() => buildVolleyballStandings(), []);
  const filteredBasketball = basketballStandings.filter((r) => r.group === bFilter);

  function renderStandings() {
    if (activeSport === "Volleyball") return <TableBlock title="Volleyball Standings (Auto)" rows={volleyballStandings} />;
          ...
          <div className="flex flex-wrap gap-2">
            {(["U15 Boys", "U15 Girls"] as BFilter[]).map((f) => (
              <button key={f} onClick={() => setBFilter(f)}
                className={clsx("rounded-full border px-3 py-1 text-xs transition", bFilter === f ? "border-cyan-400/50 bg-cyan-400/20 text-cyan-100" : "border-white/10 bg-white/5 text-slate-200 hover:bg-white/10")}>
                {f}
              </button>
            ))}
          </div>
          <TableBlock title="Basketball Standings (Auto)" rows={filteredBasketball} />
        </div>
      );
    }
    return <div className="rounded-xl border border-white/10 bg-black/20 p-4 text-slate-300">{activeSport} standings coming soon.</div>;
  }
  ...
  function renderContent() {
    ...
    if (activeSection === "Players") return <div className="rounded-xl border border-white/10 bg-black/20 p-4"><Link href={`/players?sport=${activeSport.toLowerCase()}`} className="inline-block rounded-lg border border-white/15 bg-white/10 px-3 py-1.5 text-sm hover:bg-white/20">Open Players</Link></div>;
  ...
  return (
    <div className="mt-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setCollapsed((v) => !v)} className="rounded-lg border border-white/15 bg-white/10 px-3 py-1.5 text-xs hover:bg-white/20">
          {collapsed ? "Expand Top Panel" : "Collapse Top Panel"}
        </button>
      </div>

      {!collapsed && (activeSport ? (
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
          <div className="mb-3"><span className="rounded-full border border-[#F26A3D]/40 bg-[#F26A3D]/15 px-2 py-0.5 text-xs text-orange-100">{leagueBySport[activeSport]}</span></div>
        ...
        <table className="min-w-full text-sm">
          <thead className="text-slate-400">
            <tr className="border-b border-white/10">
              <th className="py-2 text-left">Team</th><th className="py-2 text-left">Group</th><th className="py-2 text-left">P</th><th className="py-2 text-left">W</th><th className="py-2 text-left">L</th><th className="py-2 text-left">PF</th><th className="py-2 text-left">PA</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => (
              <tr key={`${r.team}-${r.group}-${i}`} className="border-b border-white/5">
                <td className="py-2">{r.team}</td><td className="py-2">{r.group}</td><td className="py-2">{r.played}</td><td className="py-2">{r.wins}</td><td className="py-2">{r.losses}</td><td className="py-2">{r.pointsFor}</td><td className="py-2">{r.pointsAgainst}</td>
              </tr>
            ))}
          </tbody>
        </table>
